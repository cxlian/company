 ...
 * Msw.util.loadJS.load(url, resultFN, faultFN);
 *
 * @param {String} url 要加载的JS文件
 * @return {boolean} 返回是否已经加载过
 * Msw.util.ajaxJS.isLoad(url);
 */
Msw.util.loadJS = function(){
	var jss, ajax, isLoad, nav, isIE, run,
	...
	return {
		load: function(url, resultFN, faultFN, running, async){
			if(url == null || url == "" || typeof url!='string'){resultFN('ok'); return true;}
			var urls = url.split(";");
			Msw.util.loadJS.loads(urls, resultFN, faultFN, running);
		...
		loadSup: function(ec, resultFN, faultFN, running, async){
			...
			if(!e && resultFN) resultFN();
				...
				var ls = function(sup){
					if(sup.sup){
						Msw.util.loadJS.loadSup(sup.sup,function(){
