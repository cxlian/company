function Page(opt){
		...
		var n = 0,htm = '';
		var clickpages = {//点击页码的类
			elem:set.elem,//传入的dom盒子
			num:set.num,//页码数
			callback:set.callback,//回调函数
			init:function(){//初始化
				this.elem.children('li').click(function () {//this.elem即传入的dom dom下的li点击执行
					var txt = $(this).children('a').text();//获取点击该a标签的文本内容
					var page = '', ele = null;
					var page1 = parseInt(clickpages.elem.children('li.active').attr('page'));//当前所在页的li的page属性的值
					if (isNaN(parseInt(txt))) {//如果当前点击的值不是数字
						switch (txt) {//判断文本
							case '›':
					...
					if (page != '' && page != page1) {//页码不为空，并且不等于原来的页面
						if (clickpages.callback){//点击类的回调函数
							clickpages.callback(parseInt(page));//返回当前页
						}
					}
				});
			},
