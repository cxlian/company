...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
    ...
    init : function(sb){
        sb.on('render', function(){
            ...
            if(this.form){
                Ext.getCmp(this.form).on('beforedestroy',function(){this.hideErrors();}, this);
    ...
    getMsgEl : function(){
        if(!this.msgEl){
            ...
            this.msgEl.on('click', function(e){
                ...
                if(t){
                	var myerr = Ext.getCmp(t.id.split('x-err-')[1])
                    if(myerr){
                    	myerr.focus();
                    	if(!Ext.isEmpty(myerr.getEl())) myerr.getEl().highlight();
                    }
                    this.hideErrors();
                }
